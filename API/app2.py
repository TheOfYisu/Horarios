...
def getAllpersonal():
    try:
        cur = mysql.connection.cursor()
        cur.execute('SELECT nombre,Apellido,Numero_identidad,Telefono,Correo,estado,Genero,tipodeusuario,turno,horas_al_dia,horas_al_mes FROM `personal` JOIN horario on Numero_identidad=id_empleado JOIN credenciales on Numero_identidad=ide_personal;')
        rv = cur.fetchall()
        cur.close()
        payload = []
        content = {}
...
def getALLdepartamento(id):
    try:
        ...
        variableSTR=variableSTR.replace(",",'')
        rv1=variableSTR
        cur.execute(f'SELECT Nombre,Apellido,Numero_identidad,Telefono,Correo,Genero,cargo FROM personal JOIN empleados on Numero_identidad=id_empleado_e JOIN jefe on dpto_nombre="{rv1}" GROUP BY Numero_identidad')
        rv = cur.fetchall()
        cur.close()
        payload = []
        content = {}
...
def getAllestadoDEP(id):
    try:
        ...
        variableSTR1=variableSTR1.replace(",",'')
        rv2=variableSTR1
        cur.execute(f'SELECT nombre,Apellido, numero_identidad,estado FROM `personal` JOIN empleados on Numero_identidad=id_empleado_e AND dpto_nombre="{rv2}" GROUP BY Numero_identidad;')
        rv = cur.fetchall()
        cur.close()
        payload = []
        content = {}
...
def getAllByIdpersonal(id):
    try:
        cur = mysql.connection.cursor()
        cur.execute(f'SELECT nombre,Apellido,Numero_identidad,Telefono,Correo,estado,Genero,cargo,turno,horas_al_dia,horas_al_mes FROM `personal` JOIN horario on Numero_identidad=id_empleado JOIN credenciales on Numero_identidad="{id}" JOIN empleados on id_empleado_e=numero_identidad GROUP BY Numero_identidad;')
        rv = cur.fetchall()
        cur.close()
        payload = []
        content = {}
...
def getAlljefe():
    try:
        cur = mysql.connection.cursor()
        cur.execute('SELECT nombre,Apellido,Numero_identidad,Telefono,Correo,estado,Genero,nom_dpto_cargo,nombrede_empleado FROM `personal` JOIN jefe on Numero_identidad=ide_empleado_j;')
        rv = cur.fetchall()
        cur.close()
        payload = []
        content = {}
...
def getAllempleado():
    try:
        cur = mysql.connection.cursor()
        cur.execute('SELECT nombre,Apellido,Numero_identidad,Telefono,Correo,Genero,cargo,dpto_nombre,ide_empleado_J FROM `personal` JOIN empleados on Numero_identidad=id_empleado_e JOIN jefe on dpto_nombre=nom_dpto_cargo;')
        rv = cur.fetchall()
        cur.close()
        payload = []
        content = {}
...
def getempleadoByIdjefe(id):
    try:
        cur = mysql.connection.cursor()
        cur.execute(f'SELECT nombre,Apellido,Numero_identidad,Telefono,Correo FROM personal JOIN empleados on Numero_identidad=id_empleado_e JOIN jefe on ide_empleado_j="{id}" AND nom_dpto_cargo=dpto_nombre;')
        rv = cur.fetchall()
        cur.close()
        payload = []
        content = {}
...
def getAdmin():
    try:
        cur = mysql.connection.cursor()
        cur.execute('SELECT correo FROM `personal` JOIN credenciales on Numero_identidad=ide_personal WHERE tipodeusuario="Admin"')
        rv = cur.fetchall()
        cur.close()
        payload = []
        content = {}
